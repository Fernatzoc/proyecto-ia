...
def take_input(state, bot_move):
    ...
    if all_open_flag:
        ...
        box_dict = {0: "Arriba a la izquierda", 1: "Centro superior", 2: "Arriba a la derecha",
                    3: "Centro izquierda", 4: "Centro", 5: "Centro derecha",
                    6: "Abajo a la izquierda", 7: "Centro de abajo", 8: "Abajo a la derecha"}
        print("Dónde le gustaría colocar 'X' en ~"
              + box_dict[next_box(bot_move)] + "~ Cuadro?")
    x = int(input("Fila = "))
    if x == -1:
        raise SystemExit
    y = int(input("Columna = "))
...
def game(state="." * 81, depth=20):
    ...
    while True:
        ...
        print("Por favor, espera, Bot está pensando...")
        s_time = time()
        bot_state, bot_move = minimax(user_state, user_move, "O", depth,
                                      s_time)

        print("#" * 40)
        print("Bot coloca 'O' en", bot_move, "\n")
